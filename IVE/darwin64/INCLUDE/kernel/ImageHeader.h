...
typedef struct ImageHeader {
	...
	struct {
	    int8_t exttype[4];
	    int32_t nversion;
	    int8_t extra[IMAGE_EXTRA_COUNT - 8];
	} e;
    } va1;
    int16_t nint;
    int16_t nreal;
...
/*
 * On the assumption that p_hdr is in the IW_EM0_HEADER format, returns a
 * nonzero value if the file version number indicates that the exttype field
 * is present.  Otherwise, returns zero.
 */
int IMhas_exttype_field(const ImageHeader* p_hdr);
