...
/*
 * Define how padding is dones for types that that span fractions of bytes.
 * Used by IWRtPixSizeExt() and, internally, by IMcompute_pixel_size_ext().
 */
#define  IW_BIT_PAD_SAMPLE        0 /* Pad by sample or no padding necessary */
#define  IW_BIT_PAD_ROW           1 /* Pad each row to a full # of bytes */
#define  IW_BIT_PAD_SECTION       2 /* Pad each section (one full of rows and
				       columns) to a full # of bytes */
...
/*
 * These are the allowed values for the style code used by
 * IWAlSecNumDisplay/IWRtSecNumDisplay.
 */
#define IW_SECNUM_OFF             0
#define IW_SECNUM_ZWT_LL          1 /* Z/W/T in lower left corner */

#define IW_PRIISM0_HEADER (0)
...
#define IW_EM0_HEADER (1)
typedef struct IW_EM_Header {
  int nx,ny,nz;
  int mode;
  int nxst, nyst, nzst;
  int mx, my, mz;
  ...
  short nint,nreal;
  short nres,nzfact;
  char extra2[24];         /* With the MRC2014 format, the first 8 bytes of
                              are used to indicate the extended header format
                              and file format version number. */
  short file_type, lens, n1, n2, v1, v2;
  char extra3[12];
  float tilt_x, tilt_y, tilt_z;
  float x0, y0, z0;
  char map[4];
  ...
  int nlab;
  char label[800];
} IW_EM_HEADER, *IW_EM_HEADER_PTR;

/*
 * These are return values for IMRtExHdrFmt():  define how the extended
 * header information should be interpreted.
 */
#define IW_EXTHDR_PRIISM0 (0) /* # of ints, # of floats per section */
#define IW_EXTHDR_CRYSTAL (1) /* crystallography symmetry operators */
#define IW_EXTHDR_UNKNOWN (-1) /* is not recognized by the IM library */
